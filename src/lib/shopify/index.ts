...
import { checkOutMutation } from './mutations/check-out';
   ...
   const collections = [
      {
         ...
         title: 'All',
         description:
            'All products of Khpal Store which includes watches, audio devices, baby care, electronics and fashion items',
         seo: {
            title: 'All',
            description:
               'All products of Khpal Store which includes watches, audio devices, baby care, electronics and fashion items'
...
export async function getMetaObject({ handle, type }: { handle: string; type: string }) {
...
export async function getProductMetaFields({ handle, key }: { handle: string; key: string }) {
   const res = await shopifyFetch<any>({
      query: getProductMetaFieldsQuery,
      variables: { handle, key }
   });

   return res.body.data.product.metafields[0];
}
...
export async function getSliderImages(sliderHandle: string) {
   ...
   data = data.references.nodes.map((obj: any) => obj.fields);
   data = data.map((arr: any) => {
      const [imageObj] = arr.filter((obj: any) => obj.key === 'image');
      return imageObj.reference.image;
   });
